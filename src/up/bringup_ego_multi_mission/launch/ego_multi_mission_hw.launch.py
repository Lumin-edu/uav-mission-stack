...
坐标系约定：
  ...
  - 多航点文件使用任务系 (x右,y前,z上)，runner 内部自动转 ROS 系
"""
...
def generate_launch_description():
    ...
    return LaunchDescription(
        [
            ...
            # goal_z is still passed to EGO's manual-goal fallback parameter,
            # but no single startup goal node is created in this package.
            DeclareLaunchArgument("goal_z", default_value="0.8"),
            DeclareLaunchArgument("start_with_waypoints", default_value="true"),
            DeclareLaunchArgument(
                "route_file",
                default_value=os.path.join(ego_hw_share, "config", "multi_waypoints.yaml"),
            ),
