...
"""将 Point-LIO 里程计转换为 PX4 ``VehicleOdometry`` 视觉里程计输入。

输入位姿首先通过固定外参从 MID360 IMU 原点 ``base_link`` 补偿到机体中心
``base``，然后以首次有效位姿为参考，将 ROS/Point-LIO 世界坐标中的相对运动
旋转到 PX4 NED。位置、速度和航向分别按各自语义转换：位置需要减参考原点并
加 PX4 原点，速度只允许旋转，航向则单独处理符号和初始偏置。

该节点只向 PX4 提供外部视觉里程计测量，不发布 Offboard 控制指令；是否融合
位置、航向和速度由配置及 PX4 EKF 参数共同决定。
"""
...
class PointlioToPx4VisualOdom(Node):
    ...
    def __init__(self) -> None:
        ...
        # 可只捕获 PX4 初始航向来旋转任务坐标轴，而不要求使用 PX4 位置作平移对齐；
        # 该模式用于保持已验证方形任务的路径方向。
    ...
    def align_if_ready(
        ...
        """在所需 PX4 参考有效后，一次性锁定位置原点和世界到 NED 的航向偏置。

        若 ``use_px4_reference`` 关闭，Point-LIO 初始位置映射到 NED 原点；若开启，
        则映射到当前 PX4 本地位置。航向偏置单独计算，不应混入三轴位置旋转矩阵。
        """
        if self.aligned:
            return True
    ...
    def position_is_safe(self, pos: tuple[float, float, float]) -> bool:
        """拒绝非有限值、异常高度和单帧大跳变，避免污染 PX4 EKF。"""
    ...
    def pointlio_position_to_px4_ned(self, x: float, y: float, z: float) -> tuple[float, float, float]:
        """将补偿后的 base 位置从 Point-LIO 世界系映射到 PX4 本地 NED。

        位置必须先减去 Point-LIO 初始原点，再做轴向和航向旋转，最后加上 PX4
        参考原点；这是仿射变换，与下面不含平移项的向量变换不同。
        """
    ...
    def yaw_to_px4_quaternion(self, pointlio_yaw: float) -> list[float]:
        """按配置的符号和对齐偏置生成 PX4 使用的 ``wxyz`` 偏航四元数。"""
        yaw = self.wrap_angle(
    ...
    def pointlio_callback(self, msg: Odometry) -> None:
        """执行完整转换，并仅在时间、对齐和位置安全检查通过后发布。"""
        # 第一步先做 Point-LIO base_link（IMU 原点）-> base（机体中心）补偿。
        # 后续初始零点、NED 转换和 PX4 输出全部使用 vehicle_position /
        # vehicle_orientation，不再使用原始 IMU 位姿。
