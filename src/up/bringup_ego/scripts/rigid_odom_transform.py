...
class RigidOdomTransform(Node):
    def __init__(self) -> None:
        ...
        self.target_topic = str(
            self.declare_parameter("target_topic", "/ego/odom_base").value
        )
        self.target_child_frame = str(
            self.declare_parameter("target_child_frame", "base").value
        )
        self.translation = self._vector_parameter(
            "base_link_to_base_translation", [-0.011, -0.02329, -0.05588], 3
        )
        self.rotation = normalize_quaternion(
        ...
        self.subscription = self.create_subscription(
            Odometry, self.source_topic, self.odom_callback, 10
        )
        self.get_logger().info(
            f"Rigid odometry: {self.source_topic} -> {self.target_topic}, "
            f"child_frame_id={self.target_child_frame}, "
    ...
    def odom_callback(self, msg: Odometry) -> None:
        ...
        if not all(math.isfinite(value) for value in position + orientation):
            self.get_logger().warn("Dropping non-finite transformed odometry")
            return
