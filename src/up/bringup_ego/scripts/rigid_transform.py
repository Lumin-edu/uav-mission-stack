...
def rotate_vector(quaternion: Sequence[float], vector: Sequence[float]) -> Vector3:
    ...
    # q * [v, 0] * q^-1, expanded to avoid temporary quaternion allocations.
    tx = 2.0 * (qy * vz - qz * vy)
    ty = 2.0 * (qz * vx - qx * vz)
    tz = 2.0 * (qx * vy - qy * vx)
    return (
...
def compose_pose(
    ...
    """Return T_world_child = T_world_parent * T_parent_child (ROS xyzw)."""
    px, py, pz = _finite_vector(world_parent_position, 3, "world_parent_position")

    # The lever arm is defined in base_link (MID360 IMU) coordinates. Rotate it
    # by the current world orientation before adding it to the world position.
