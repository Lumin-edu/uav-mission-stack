...
int init_map_size = 100, con_frame_num = 1;
double match_s = 81, satu_acc, satu_gyro, cut_frame_time_interval = 0.1;
float plane_thr = 0.1f;
double filter_size_surf_min = 0.2, filter_size_map_internal = 0.5, filter_size_map_publish = 0.8,
       filter_size_map_save = 0.2, fov_deg = 180;
...
bool runtime_pos_log, pcd_save_en, path_en, save_on_shutdown = true, extrinsic_est_en = true;
double pcd_save_period_sec = 0.0;
std::string pcd_save_path = "PCD/scans.pcd";
bool scan_pub_en, scan_body_pub_en, tf_send_en, map_pub_en = true;
int map_publish_interval = 20;
std::string odom_topic = "odom";
std::string cloud_registered_topic = "cloud_registered";
std::string cloud_registered_body_topic = "cloud_registered_body";
std::string map_topic = "map_cloud";
std::string path_topic = "path";
std::string odom_frame_id = "odom";
std::string base_frame_id = "base_link";
std::string cloud_registered_frame_id = "odom";
std::string cloud_registered_body_frame_id = "base_link";
...
void readParameters(std::shared_ptr<rclcpp::Node> & nh)
{
  ...
  try {
    ...
    nh->declare_parameter<int>("point_filter_num", 3);
    ...
    nh->declare_parameter<double>("filter_size_map_internal", 0.5);
    nh->get_parameter("filter_size_map_internal", filter_size_map_internal);

    nh->declare_parameter<double>("filter_size_map_publish", 0.8);
    nh->get_parameter("filter_size_map_publish", filter_size_map_publish);

    nh->declare_parameter<double>("filter_size_map_save", 0.2);
    nh->get_parameter("filter_size_map_save", filter_size_map_save);
    ...
    nh->declare_parameter<bool>("publish.map_publish_en", true);
    nh->get_parameter("publish.map_publish_en", map_pub_en);

    nh->declare_parameter<int>("publish.map_publish_interval", 20);
    nh->get_parameter("publish.map_publish_interval", map_publish_interval);

    nh->declare_parameter<bool>("publish.tf_send_en", true);
    nh->get_parameter("publish.tf_send_en", tf_send_en);

    nh->declare_parameter<std::string>("publish.odom_topic", "odom");
    nh->get_parameter("publish.odom_topic", odom_topic);

    nh->declare_parameter<std::string>("publish.cloud_registered_topic", "cloud_registered");
    nh->get_parameter("publish.cloud_registered_topic", cloud_registered_topic);

    nh->declare_parameter<std::string>(
      "publish.cloud_registered_body_topic", "cloud_registered_body");
    nh->get_parameter("publish.cloud_registered_body_topic", cloud_registered_body_topic);

    nh->declare_parameter<std::string>("publish.map_topic", "map_cloud");
    nh->get_parameter("publish.map_topic", map_topic);

    nh->declare_parameter<std::string>("publish.path_topic", "path");
    nh->get_parameter("publish.path_topic", path_topic);

    nh->declare_parameter<std::string>("frame.odom_frame_id", "odom");
    nh->get_parameter("frame.odom_frame_id", odom_frame_id);

    nh->declare_parameter<std::string>("frame.base_frame_id", "base_link");
    nh->get_parameter("frame.base_frame_id", base_frame_id);

    nh->declare_parameter<std::string>("frame.cloud_registered_frame_id", "odom");
    nh->get_parameter("frame.cloud_registered_frame_id", cloud_registered_frame_id);

    nh->declare_parameter<std::string>("frame.cloud_registered_body_frame_id", "base_link");
    nh->get_parameter("frame.cloud_registered_body_frame_id", cloud_registered_body_frame_id);

    nh->declare_parameter<std::string>("frame.map_frame_id", "odom");
    nh->get_parameter("frame.map_frame_id", map_frame_id);
    ...
    nh->declare_parameter<bool>("pcd_save.save_on_shutdown", true);
    nh->get_parameter("pcd_save.save_on_shutdown", save_on_shutdown);

    nh->declare_parameter<double>("pcd_save.save_period_sec", 0.0);
    nh->get_parameter("pcd_save.save_period_sec", pcd_save_period_sec);

    nh->declare_parameter<std::string>("pcd_save.save_path", "PCD/scans.pcd");
    nh->get_parameter("pcd_save.save_path", pcd_save_path);
