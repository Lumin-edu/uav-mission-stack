...
### 1.3. **livox_ros_driver2 (ROS1)**
Follow [livox_ros_driver2 Installation](https://github.com/Livox-SDK/livox_ros_driver2).

*Remarks:*
- Since the FAST-LIO must support Livox serials LiDAR firstly, the ROS1 **livox_ros_driver2** package must be built and sourced before running any FAST-LIO launch file.
- How to source? Run ``` source $Livox_ros_driver_dir$/devel/setup.bash ``` after building the Livox driver workspace (or source this workspace's ``` devel/setup.bash ``` when both packages are built together).
...
```
    ...
    roslaunch livox_ros_driver2 msg_AVIA2.launch
```
- For Livox serials, FAST-LIO requires the custom message published by ``` msg_AVIA2.launch ```; its ``` livox_ros_driver2/CustomMsg ``` data includes each point's timestamp, which is required for motion undistortion.
- If you want to change the frame rate, modify the **publish_freq** parameter in the corresponding launch/configuration file of [livox_ros_driver2](https://github.com/Livox-SDK/livox_ros_driver2) before building the driver package.
