...
namespace ego_planner
{
  ...
  void EGOReplanFSM::checkCollisionCallback()
  {
    ...
    if (map->getOdomDepthTimeout())
    {
      ...
      // Force one stationary B-spline to be emitted by the FSM on the next
      // execution tick, then remain stopped until the operator restarts the
      // mission with a healthy sensor stream.
      flag_escape_emergency_ = true;
