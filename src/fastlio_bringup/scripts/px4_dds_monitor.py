...
class Px4DdsMonitor(Node):
    def __init__(self) -> None:
        super().__init__("fastlio_px4_dds_monitor")
        qos = QoSProfile(
        ...
        self.distance_sensor_topic = str(
            self.declare_parameter("distance_sensor_topic", "/fmu/out/distance_sensor").value
        )
        ...
        self.create_subscription(VehicleStatus, self.vehicle_status_topic, self.vehicle_status_callback, qos)
        ...
        self.create_subscription(DistanceSensor, self.distance_sensor_topic, self.distance_sensor_callback, qos)
        self.create_timer(2.0, self.timer_callback)
        self.get_logger().info(
            "Monitoring PX4 DDS for FAST-LIO hover: "
            f"status={self.vehicle_status_topic}, local_position={self.vehicle_local_position_topic}, "
    ...
    def timer_callback(self) -> None:
        self.get_logger().info(
            "PX4 DDS status: "
            f"vehicle_status={self.status_received}, local_position={self.local_position_received}, "
            f"distance_sensor={self.distance_sensor_received}, nav_state={self.last_nav_state}, "
            f"xy_valid={self.last_xy_valid}, z_valid={self.last_z_valid}, "
            f"dist_bottom_valid={self.last_dist_bottom_valid}, dist_bottom={self.last_dist_bottom}, "
